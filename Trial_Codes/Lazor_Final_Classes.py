...
class Grid():
    ...
    def blocks(self):
        ...
        for i in range((A_blocks + B_blocks), (A_blocks + B_blocks + C_blocks)):
            movable_blocks[i] = 'C'
        O_blocks = len(movable_blocks) - (A_blocks + B_blocks + C_blocks)
        MAX_BOARDS = int(math.factorial(len(movable_blocks)) / O_blocks)
        if A_blocks != 0:
        ...
        while ITER_B <= MAX_BOARDS:
            ...
            for i in range(length):
                for j in range(width):
                    if possible_board.board[i][j] == 'o':
                        possible_board.board[i][j] = permut[counter]
                        counter += 1
            grid = possible_board.create_grid()
            ITER_B += 1
            if lazor_path(grid, lazors, sinks):
                ...
                for y in grid:
                    ...
                    print()
                break
            del(possible_board) # to delete the class object so created (reduce space)
        print(ITER_B)
...
if __name__ == "__main__":
    board_given, A_blocks, B_blocks, C_blocks, lazors, hole = read_bff("mad_1.bff")
    print(lazors)
    print(hole)
