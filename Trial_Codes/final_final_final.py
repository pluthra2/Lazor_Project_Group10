...
def GUI_board(grid, filename, lazors_ori, holes, stack_lazors, original_board, blockSize=100):
    name = filename.split('.')
    file_name = name[0]
    w_blocks = len(grid[0])
    h_blocks = len(grid)
    ...
    for i in holes:
        ...
        a = x1 * 50
        b = y1 * 50
#        draw.rectangle([a - 6, b - 6, a + 6, b + 6], fill=(232,27,27))
        ...
        ** Returns **
        ...
        for permut in permutations:
            ...
            if Result:
                ...
                GUI_board(final_board, filename, self.L, self.H, stack_lazors, self.board)
