        ...
        ** Returns **
        ...
        for permut in permutations:
            ...
            if lazor_path(possible_grid, self.L, sinks):
                ...
                q = Remove_grid(possible_grid)
                save_maze(q, filename, self.L, self.H)
...
def save_maze(grid, filename, lazors_ori, holes, blockSize=50):
    name = filename.split('.')
    a=name[0]
    w_blocks = len(grid[0])
    h_blocks = len(grid)
    ...
    for x in range (50, SIZE[0]+50, 50):
        x1=x
        y1=0
        x2=x
        y2=SIZE[1]
    ...
    for y in range (50, SIZE[1]+50, 50):
        y1=y
        x1=0
        y2=y
        x2=SIZE[0]
    ...
    for i in holes: 
        ...
        a=x1*25
        b=y1*25
        draw.rectangle([a-5,b-5,a+5,b+5], fill=128)
    ...
    for j in lazors_ori:
        ...
        a=a1*25
        b=b1*25
        draw.ellipse([a-3,b-3,a+3,b+3], fill=128)
        
    for i, element in enumerate(grid):
        for j, name in enumerate(element):
            if j == 'x':
                print ("jkhgf")
...
#    print (new_grid)
    newer_grid=[[] for i in new_grid]
    for x in range (len(new_grid)):
        for y in range(len(new_grid[0])):
            if y%2!=0:
                newer_grid[x].append(new_grid[x][y])
#    print (newer_grid)
    return newer_grid
