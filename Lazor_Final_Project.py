...
def GUI_board(grid, filename, lazors_ori, holes, stack_lazors, blockSize=50):
    name = filename.split('.')
    file = name[0]
    w_blocks = len(grid[0])
    h_blocks = len(grid)
    ...
    def blocks(self, filename):
        ...
        ** Returns **
        ...
        for permut in permutations:
            ...
            ITER_B += 1
            if lazor_path(possible_grid, self.L, sinks)[0]:
                ...
                GUI_board(final_board, filename, self.L, self.H, lazor_path(possible_grid, self.L, sinks)[1])
